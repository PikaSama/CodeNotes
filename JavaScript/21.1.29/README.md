...
2. 任何值与**字符串**做**加法**运算，会将其转换为 String 类型，并执行**字符串拼接**操作
...
运算规则：符号两侧的值**都为 true**时，返回**true**

也就是说，如果符号两侧中**任意**一侧的值为**false**，则返回**false**

短路与相比与在逻辑上做了优化：

-   如果第一侧的值为**true**，继续运算另一侧的值
-   如果第一侧的值为**false**，**不继续运算**另一侧的值，直接返回**false**
...
运算规则：符号两侧中**任意**一侧的值为**true**，返回**true**

也就是说，如果符号两侧的值**都为 false**，才返回**false**

短路或相比或在逻辑上做了优化：

-   如果第一侧的值为**false**，继续运算另一侧的值
...
-   短路与运算：
    1. 如果两个值**都是 true**，将**第二侧**的值作为返回值
    2. 如果两个值的**其中**一个值是 false，将**这一侧**的值作为返回值
    3. 如果两个值**都是 false**，将**第一侧**的值作为返回值
